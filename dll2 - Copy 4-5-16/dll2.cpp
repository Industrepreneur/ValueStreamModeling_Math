...
BOOL Cdll2App::InitInstance()
{
 ...
 char s2 [MESSAGE_SIZE];
 char s3 [MESSAGE_SIZE];
	...
	for (i=0; i<eq_count; i++) { 
	  ...
	  teq->facovt = e_x5[i];
	  sprintf(s2, "%i", e_x6[i]);
	  teq->lab_ptr = c1->find_lab_name(s2);
	  ...
	  if (teq->lab_ptr  == -1) { 
		  sprintf(buf1, "INTERNAL ERROR: #(6) \n Equipment uses an unknown Labor Group ");
		  warn_err ( c1, buf1, 0, s2,  s1,   sNULL, sNULL, sNULL, sNULL);
		  c1->CALC_CANCEL = true;
	...
	for (i=0; i<op_count; i++) { 
	  ...
	  sprintf_s(s3, "%i", o_x1[i]);  //  change num into string...
	  strcpy(toper->oldname, s3);
	 
	  sprintf_s(s2, "%i", o_x3[i]);  //  change num into string...
	  toper->part_ptr =  c1->find_part_name(s2);
	  if (toper->part_ptr  == -1) { 
		  sprintf(buf1, "INTERNAL ERROR: #(7) \n Operation is tied to an unknown Product ");
		  warn_err ( c1, buf1, 0, sNULL,  sNULL,   s2, s3, sNULL, sNULL);
		  c1->CALC_CANCEL = true;
	  ...
	  if (toper->eq_ptr  == -1) { 
		  sprintf(buf1, "INTERNAL ERROR: #(8) \n Operation is tied to an unknown Equipment group ");
		  warn_err ( c1, buf1, 0, sNULL,  s1,   s2, s3, sNULL, sNULL);
		  c1->CALC_CANCEL = true;
	  ...
	  toper->opnum = o_x2[i];
		 ...
		 sprintf_s(s1, "%i", o_x2[i]);  //  change num into string...
write_check(c1->varlocal, s1);
write_check(c1->varlocal, s1);
sprintf_s(s1, "%i", toper->opnum);  //  change num into string...
write_check(c1->varlocal, s1);
	  ...
	  toper->lsize = c1->get_part_num(toper->part_ptr)->lotsiz; //  gwwd fix 10-14 
	}
	...
	for (i=0; i<r_count; i++) { 
	  ...
	  sprintf_s(s1, "%i", r_x2[i]);  //  change num into string...  route ptr
	   ///   error is here !!!
	  if (troute->part_ptr  == -1) { 
		   sprintf(buf1, "INTERNAL ERROR: #(9) \n Routing is tied to an unknown Product ");
		  warn_err ( c1, buf1, 0, sNULL,  sNULL,   s1, sNULL, sNULL, sNULL);
		  c1->CALC_CANCEL = true;
	  ...
	  sprintf_s(s3, "%i-%i", r_x1[i], r_x2[i]);  //  change num into string...
	  troute->op_from_ptr = c1->find_oper_name (s3);
	  sprintf_s(s3, "%i",  r_x2[i]);
	    // sprintf(s1, "  route from op  %i %i %i ", i, r_x2[i], troute->op_from_ptr);
        // write_check(c1->varlocal, s1);
	   ///   error is here !!!
	  if (troute->op_from_ptr  == -1) { 
		  sprintf(buf1, "INTERNAL ERROR: #(10) \n Routing FROM is tied to an unknown Operation");
		  warn_err ( c1, buf1, 0, sNULL,  sNULL,   s1, s3, sNULL, sNULL);
		  c1->CALC_CANCEL = true;
		 
	  }
	  
	  sprintf_s(s3, "%i-%i", r_x1[i], r_x3[i]);  //  change num into string...
	  troute->op_to_ptr = c1->find_oper_name (s3);
	  sprintf_s(s3, "%i",  r_x3[i]);


	  ///   error is here !!!
	  if (troute->op_to_ptr  == -1) { 
		   sprintf(buf1, "INTERNAL ERROR: #(11) \n Routing TO is tied to an unknown Operation");
		  warn_err ( c1, buf1, 0, sNULL,  sNULL,   s1, s3, sNULL, sNULL);
		  c1->CALC_CANCEL = true;
...
void write_check2(char * varlocal, char *  s0) {
     ...
     //xx1 = varlocal;
	 //xx1 += "\\";
	 sprintf(s2, "%s\\calc_cv.txt", varlocal);
    // xx1 += s2;
...
write_check(c1->varlocal, "about to test opnum ");

					toper = c1->get_oper_num(i_oper);
					sprintf(buf1, " part name %s  icount %i  %i, %s ", tpart->name, i, toper->opnum, toper->name );
					write_check(c1->varlocal, buf1);


                    // USING OLD NAME = OP_ID in the access code !!!
                    sprintf_s(s0, "%d, %s, %i, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f \n",
                        c1->WID, tpart->name,
                        toper->opnum,
   ...
   write_check(c1->varlocal, "in to clear c1 before oper 1");
   ...
   for(toper  = c1->first_oper_ptr(); toper != NULL; toper = c1->next_oper_ptr()) { 
	...
	write_check(c1->varlocal, "in to clear c1  after route ");
	if (c1->ibom_count >0) {
	 ...
	 for(i=0; i<c1->part_count; i++) {
		 ...
		 for(i=0; i<c1->oplst_count; i++) {  //  gwwd 3-23-15
		   delete(c1->all_oplst[i]);
   }
	 
	write_check(c1->varlocal, "in to clear c1  after extras  ");
...
{ 
	...
	*/
	...
	load_data (c1,  //General_time (
	 ...
	 if (c1->CALC_CANCEL == TRUE) return(-1);
	    ...
	    write_check(c1->varlocal, "after after write  about to mpcalc in run it \n");

		
		// sprintf(s1, "  time 1  + c1  %9.2f ", c1->t1);
        //  write_check(c1->varlocal, s1);

		// write_check(c1->varlocal, "about to stop  mpcalc in run it \n");
		// return(0);
	   ...
	   if(c1->CALC_CANCEL == false) { 

			write_check(c1->varlocal, "about write_results in run it \n");

			  //return(0);  //gwwd xxx 5-14-15 test 
      		write_results (c1);

	   }
	   else { 
		   	write_check(c1->varlocal, "internal error/data error ... NO write_results in run it \n");
	   }
...
{ 
		...
		ofstream clearFile (buf1);  //   app =>  append 
		//ofstream outfile ( xx1, ios::app );  //   app =>  append 
		if (clearFile.is_open()) {
			clearFile << "Starting c++ calculations \n\n" ;
			clearFile.close();
		}
