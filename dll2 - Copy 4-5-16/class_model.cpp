...
class_model::class_model(int l_size, int e_size, int p_size, int o_size, int r_size, int i_size, char * dir1)
{
	...
	all_lab = new class_lab* [l_size+1];
	...
	all_part= new class_part  * [p_size+1];
	for(i=0;i<p_size;i++) { 
		all_part[i] = new class_part();
	}
	part_count = p_size;
	part_point = 0;
	all_part[p_size] = NULL;
	...
	all_oper= new class_oper  * [o_size+1];
	for(i=0;i<o_size;i++) { 
	...
	all_oper[o_size]= NULL;
	oper_count = o_size;
	oper_point = 0;

	
	all_route= new class_route  * [r_size+1];
	for(i=0;i<r_size;i++) { 
	...
	all_route[r_size] = NULL;
	route_count = r_size;
	route_point = 0;

	
	all_ibom= new class_ibom  * [i_size+1];
	for(i=0;i<i_size;i++) { 
	...
	ibom_count = i_size;
	ibom_point = 0;
	all_ibom[i_size] = NULL;
	
	all_wused= new class_wused  * [p_size+1];
	for(i=0;i<p_size;i++) { 
	...
	all_wused[part_count] = NULL;
	wused_point = -1;


	//--------------------------------------------------------------
	all_oplst = new class_oplst  * [o_size+1];
	...
	all_oplst[o_size] = NULL; //  gwwd  3-25-2015  aDDED    
