	// dll3.cpp : Defines the initialization routines for the DLL.
//
...
BEGIN_MESSAGE_MAP(Cdll3App, CWinApp)
END_MESSAGE_MAP()


// Cdll3App construction
...
// The one and only Cdll3App object
...
// Cdll3App initialization

BOOL Cdll3App::InitInstance()
{
 ...
 char s1 [MESSAGE_SIZE];
	...
	sprintf_s(s2, "%s\\errors.txt", c1->varlocal);
...
void write_check(char * varlocal, char *  s0) {
     ...
     //xx1 = varlocal;
	 //xx1 += "\\";
	 sprintf_s(s2, "%s\\errors.txt", varlocal);
    // xx1 += s2;
...
{ 
	...
	*/
	...
	// write_check(dir1, "after load data 5\n");
	 ...
	 write_check(c1->varlocal, "at start of write_data \n");
	...
	class_oper * toper;
		toper = c1->get_oper_num(3);
		sprintf_s(buf1, "a checking where lpiece  3 %s %f \n", toper->name, toper->lpiece);
		write_check(c1->varlocal, buf1);
		toper = c1->get_oper_num(7);
		write_check(c1->varlocal, buf1);
		write_check(c1->varlocal, buf1);
		if ( toper != NULL) {

			sprintf_s(buf1, "checking where lpiece 7 %s \n", toper->name);   //  !!!!????
			write_check(c1->varlocal, buf1);
...
{ 
		...
		sprintf_s(buf1, "%s\\errors.txt", dir1 );
