...
void mnbrak(class_model * c1, float *ax, float *bx, float *cx, float *fa, float *fb, float *fc  ) //, float (*func)(float))
{
		...
		p1 = *ax;
			 ...
			 sprintf(buf1,  " in mnbrak value to f1dim ? %f ", p1);
		...
		*bx *= 100.0;
        while (vp2 < 0){
		  *bx /= 10.0;
          p2= *bx;
		     sprintf(buf1,  " in mnbrak value to f1dim 22  %f ", p2);
			 write_check(c1->varlocal, buf1);
          vp2= f1dim(c1, p2);
  ...
  */
        ...
        while (!done) {
			 sprintf(buf1,  " in mnbrak value to f1dim 33  %f ", p3);
			 write_check(c1->varlocal, buf1);
            vp3 = f1dim(c1, p3);
            if (vp3 == -1) {
		     ...
		     sprintf(buf1,  " in mnbrak value  results %f %f %f  :value of WIP: %f %f %f ", p1, p2, p3, vp1, vp2, vp3);
			 write_check(c1->varlocal, buf1);
   *ax = p1;
   *bx = p2;
   *cx = p3;
