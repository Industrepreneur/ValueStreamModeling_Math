...
/*  ******************************************************************* */
{
        ...
        for(teq  = c1->first_eq_ptr(); teq != NULL; teq = c1->next_eq_ptr())
        {
                ...
                sprintf(buf1, " eq cs2 %s %f %f %f %f ", teq->name, teq->cs2, teq->xbarbar, teq->xbard, teq->xsigbar);
				sprintf(buf2, "%s \n", buf1);
				write_check2(c1->varlocal, buf2);
...
/* ##################################################### */
{
         ...
         for(tlabor  = c1->first_lab_ptr(); tlabor != NULL; tlabor = c1->next_lab_ptr())
        {
                ...
                if ( tlabor->eq_cover == 0) goto next_lab;
                 ...
                 sprintf_s(buf1, "labor name :%s: %f %f \n", tlabor->name, tlabor->num, tlabor->num_av);
				 write_check2(c1->varlocal, buf1);
				 sprintf(buf1, "lab-val:  %f  %f  %f \n ", tlabor->num, tlabor->num_av, tlabor->eq_cover);
				 write_check2(c1->varlocal, buf1);
                // Message( c1,buf1);
                ...
                else   /* lab< eq_cover   or eq_cover = infinity */
                {
                        ...
                        u1 = MIN((0.95), tlabor->ul); //  gwwd 21-8-17  changed to .95 vs. utlimit
                         sprintf_s(buf1, "u labor %s %f %f %f", tlabor->name, u1, tlabor->num, tlabor->num_av);
						 write_check2(c1->varlocal, buf1);
                        // Message( c1,buf1);
                        ...
                        if (tlab_tpm < SSEPSILON) {
								...
								warn_err ( c1, buf1, 1,  tlabor->name,  sNULL, sNULL, sNULL, sNULL, sNULL);
                                goto next_lab;
                        }
						...
						sprintf(buf1, " tpm value %f", tlab_tpm);
						write_check2(c1->varlocal, buf1);
                        ...
                        WAIT *= nm_1;     // closed population issue
						 ...
						 sprintf_s(buf1, " nm_1 old  %f,  eq_cover %f, num_av %f, WAIT %f", nm_1, tlabor->eq_cover, tlabor->num_av, WAIT);
						 write_check2(c1->varlocal, buf1);
                        ...
                        if (tlabor->eq_cover > 0) {
							...
							//if (tlabor->num > 0 )  
						    //		WAIT = MIN(WAIT, (tlabor->eq_cover-1)/tlabor->num_av);
					        //	  else WAIT = 0 ; 
						}

						 sprintf_s(buf1, " done  %f,  eq_cover %f, num_av %f, WAIT %f", nm_1, tlabor->eq_cover, tlabor->num_av, WAIT); 
						 write_check2(c1->varlocal, buf1);
						...
						if (tlabor->balance != -1) {  
								  ...
								  for(teq  = c1->first_eq_ptr(); teq != NULL; teq = c1->next_eq_ptr()) {
									if (tlabor == c1->get_lab_num(teq->lab_ptr)) {
											...
											if (teq->num > 0)
											    //   4-23-15teq->uwait += ( tlabor->fac_eq_lab * teq->smbard ) / teq->num_av;
												if (tlabor->ul > 0.95) {
												         teq->uwait = 1.00; } else  teq->uwait += ( tlabor->fac_eq_lab * teq->smbard ) / teq->num_av;
											else
												teq->uwait += 0.0;

											  sprintf_s(buf1, "TESTING:\n \nxxxx  inside lextra  levelling...\n eqname %s %f %f \n ", teq->name, teq->uwait, tlabor->fac_eq_lab);
                                              write_check( c1->varlocal, buf1);
