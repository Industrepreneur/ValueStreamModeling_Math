...
void  write_check2(char * varlocal, char *  buf2);
...
double effabs (class_model * c1, class_lab * tlabor);
...
/* ############################################################ */
{
        ...
        */
        ...
        for(tlabor  = c1->first_lab_ptr(); tlabor != NULL; tlabor = c1->next_lab_ptr())
        {
				...
				sprintf(buf1, "labor: %s  num av %f", tlabor->name, tlabor->num_av);
				write_check2(c1->varlocal, buf1);
...
restart:
       ...
       for(tlabor  = c1->first_lab_ptr(); tlabor != NULL; tlabor = c1->next_lab_ptr())
        {
                if(tlabor->num_av > 0.0)
                {
						...
						tlabor->num_av = tlabor->num *(1.+tlabor->facovt/100.) / (1.+tlabor->max_eq_ot/100.);  // xxx gwd error 7-21-15  why max ? (eq + unused labor)
						sprintf(buf1, " labor: %s , num %f", tlabor->name, tlabor->num_av);
						write_check2(c1->varlocal, buf1);
						tlabor->eq_cover /= 100*(1.+tlabor->max_eq_ot/100.);  // xxx gwd error 7-21-15  why  =/ 100* 
         ...
         for(teq  = c1->first_eq_ptr(); teq != NULL; teq = c1->next_eq_ptr())
        {
                ...
                {  /*   --regular multiple server-- */
						...
						//  Aug 10 04 gwwd
						tlabor = c1->get_lab_num(teq->lab_ptr);
						teq->uwait *= effabs(c1, tlabor) / (1-effabs(c1,tlabor));
                        ...
                        teq->u = u1;
						teq->num_av = teq->num * (teq->facovt+100)/(100.0+c1->get_lab_num(teq->lab_ptr)->max_eq_ot); //  xxx gwd looks good  7-21-15
						sprintf(buf1, " eq %s  ave num %f ", teq->name, teq->num_av);
						write_check2(c1->varlocal, buf1);
	...
	/*  ************************************************* */
      ...
      //  MessageBox( NULL, buf1, "Into Running ? ", MB_OK | MB_TASKMODAL );
	   ...
	   write_check(c1->varlocal, "mpcalc starting mct calc !!");
                ...
                tpart->qpart   += tpart->tgather * (1.0-tpart->scrap) * tpart->lotsiz * tpart->dlam;
                   ...
                   // write_check( c1->varlocal, buf1); 
                        ...
                        for (i_oper= o_start; i_oper< o_start+o_count; i_oper++ )                
                        {
							...
							toper = c1->get_oper_num(i_oper);   //   gwwd  added .... 10-1
...
//xxZZ
                      ...
                      calc_op (c1, tpart, toper, &v1, &xbars_t2,  &xbarr_t2,  T_BATCH_WAIT_LOT);
                          ...
                          toper->w_equip += toper->vpergood * (xbarr_t1+xbars_t1 +
                           ...
                           ///  if NO DEMAND ...
							   ...
							   sprintf(buf1, "tot prt equip %s %s   %f %f ", tpart->name, toper->name, tpart->w_equip, toper->w_equip);
							   write_check2(c1->varlocal, buf1);
                               tpart->w_setup += toper->w_setup;
                               tpart->w_run   += toper->w_run  ;
							   sprintf(buf1, "tot prt run %s %s   %f %f ", tpart->name, toper->name, tpart->w_run, toper->w_run);
							   write_check2(c1->varlocal, buf1);
                               tpart->w_lot   += toper->w_lot  ;
                        /* end of new flowtimes ... */
                        tpart->tsgood  += toper->vpergood * rpv;
						sprintf(buf1, "tot prt total,rpv  %s    %f   %f %f ", tpart->name, tpart->tsgood, toper->vpergood, rpv);
						write_check2(c1->varlocal, buf1);
                        ...
                        xl_only *= (1.0 / ( 1.0 - effabs(c1,tlabor)));
             ...
             fprintf(fileout, "part , oper %s %s,  v_visit %f  X:%f xprsqr:%f  xprime:%f  x1:%f rpv:%f \n",
                ...
                tpart->tsgood  /= c1->t1;
				sprintf(buf1, "tot prt tsgood %s   %f %f ", tpart->name, tpart->tsgood, c1->t1);

                tpart->w_labor /= c1->t1;
                tpart->w_equip /= c1->t1;
				sprintf(buf1, "tot prt eq %s   %f %f ", tpart->name, tpart->w_equip, c1->t1);
				write_check2(c1->varlocal, buf1);
                tpart->w_setup /= c1->t1;
                tpart->w_run   /= c1->t1;
				sprintf(buf1, "tot prt run %s   %f %f ", tpart->name, tpart->w_run, c1->t1);
				write_check2(c1->varlocal, buf1);
                tpart->w_lot   /= c1->t1;
...
/*  ******************************************************************* */
{
        ...
        if ( xbar2 >= (xbar1-EPSILON)) {
                ...
                xm_only =  xbar2 - xbar1;
                xl_only =  xbar1 *  1.0 / ( 1.0 - effabs(c1, tlabor));

                xprime = xm_only + (xbar2 * teq->mttr/(teq->mttf)) +    ///  check gwwdcheck 10-6  down time while labor in use ? 
                    xl_only * ( 1.0 + tlabor->fac_eq_lab);

				if (c1->inOPT != 0) { 
					//sprintf(buf1, "in xprime %f ::  %f %f  : %f %f : %f : %f  %f : %f ", xprime, xbar1, xbar2, xm_only, xl_only, effabs(c1, tlabor), teq->mttr, teq->mttf, tlabor->fac_eq_lab);
				    //write_check(c1->varlocal, buf1);
				}
                return(xprime);
        }
                ...
                xprime = xbar2 * teq->mttr/(teq->mttf)  +
                       xbar2 * ( 1.0 / ( 1.0 - effabs(c1, tlabor))) * (1.0 + tlabor->fac_eq_lab);
        }
                ...
                xprime =  xbar1 * ( 1.0 / ( 1.0 - effabs(c1, tlabor))) *
...
double effabs (class_model * c1, class_lab * tlabor)
{
	...
	if (n <0)  
		x = tlabor->absrate/100.0;
		  ...
		  if (c1->inOPT != FALSE) {
	         	x = NDI_POW(tlabor->ul/100., (double) n) * tlabor->absrate/100;
		  }  else {
...
//xxZZ
        ...
        while ( (roper != NULL) && (strcmp(frm_oper->name, c1->get_oper_num(roper->op_from_ptr)->name) == 0)) {
               ...
               //  next oper oops !!!  
		//		error here !!!  same oper ? next op diff eq  gwwd   check  ; extra or necessary ? 
				
				roper = c1->get_route_num(roper->me_ptr+1);
				if (roper != NULL) {  if (tpart != c1->get_part_num(roper->part_ptr))   roper = NULL; }   //  was ; right after if
				if (roper != NULL) {  if (strcmp(frm_oper->name, c1->get_oper_num(roper->op_from_ptr)->name) !=0) roper = NULL;}  //  was ; right after if 
        }
