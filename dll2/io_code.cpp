...
{ int i;
 ...
 char buf1 [500];
    ...
    ofstream outfile ( s2  );   //  gwwd 5/5/15 
    if (outfile.is_open()) {
...
{ int i;
 ...
 ofstream errfile (xx1, ios::app );
  if (errfile.is_open()) {

     sprintf(buf2, "%d,In Calculations,%s, 1 ,%s,%s,%s,%s,%s,%s\n", level, buf1, str_l, str_e, str_p, str_o, str_r, str_i);
     errfile << buf2;
...
sprintf(s0, "GENERAL \n");
       ...
       for(teq=c1->first_eq_ptr(); teq!= NULL; teq=c1->next_eq_ptr() ) {
        ...
        sprintf_s(s0, "* NAME            " );
         outfile << s0;
        sprintf_s(s0, " EXTERNAL   PART       LOT       DEMAND      VARIABILITY    gather");
         outfile << s0;
        sprintf_s(s0, "\n");
         outfile << s0;
        ...
        sprintf_s(s0, "* NUMBER          " );
         outfile << s0;
		sprintf_s(s0, "  DEMAND    LOTSIZE    SIZE      MULTIPLIER  MULTIPLIER     tbatch");
         outfile << s0;
       ...
       for (tpart=c1->first_part_ptr(); tpart != NULL; tpart = c1->next_part_ptr()  ) {
		 ...
		 int i_oper;
			...
			//outfile.close(); //  gwwd 5-14-15 test 
			//return;
   ...
   write_check(c1->varlocal, s0);
		...
		write_check(c1->varlocal, "done parts demand, oper route " );
...
write_check(c1->varlocal, "done writing mpx.mdl");
