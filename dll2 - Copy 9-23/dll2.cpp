...
BOOL Cdll2App::InitInstance()
{
 ...
 char s1 [MESSAGE_SIZE];
	...
	for (i=0; i<l_count; i++) { 
	  ...
	  write_check(c1->varlocal, "last step ?");
  ...
  sprintf(s1, "  time 1 %10.2f ", time1);
	...
	for (i=0; i<ib_count; i++) { 
	  ...
	  tibom->cmpt_ptr = c1->find_part_name(s1);  //gwwd error !
...
void write_check(char * varlocal, char *  s0) {
     ...
     xx1 = varlocal;
	 xx1 += "\\";
	 sprintf(s2, "errors.txt");
     xx1 += s2;

         ofstream outfile ( xx1, ios::app );
    if (outfile.is_open()) {
   ...
   void write_results (class_model * c1) {
	...
	if (outfile.is_open()) {
		...
		outfile << s0;
		sprintf_s(s0,"%d, TOTAL, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f \n",  c1->WID, c1->total_pro, c1->total_shi, c1->total_scr, c1->total_wip, c1->total_ft);
         outfile << s0;
        ...
        for(tpart  = c1->first_part_ptr(); tpart != NULL; tpart = c1->next_part_ptr())
        {
         ...
         if (tpart->tgather > EPSILON) {
                ...
                sprintf_s(s0, "%d, %s, %s, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f \n",
                        ...
                        0.0,                  0.0,              0.0,            0.0,       tpart->tgather*c1->t1, 100.0,            1.0,           tpart->lotsiz * (1.0 - tpart->scrap), 1.0);
...
extern "C" int  opt_all ( int collect, char * dir1,  
               ...
               float * weight, int * optimizeLotSize, int * OptimizeTbatch, float * lotsizeValue, float * tbatchValue)
{
	...
	write_check(dir1, "before c1 start  1 \n");
...
{ 
		...
		ofstream outfile(buf1, std::ios::out);
		outfile.close();
