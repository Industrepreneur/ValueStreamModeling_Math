...
float brent(class_model * c1, float ax,float bx,float cx,    //  gwwdd  craash  code check !!!  9-27
...
float brentgd(class_model * c1, float ax,float bx,float cx,  float tol,float *xmin)
{
	...
	double fa, fb, fc;
		 ...
		 write_check(c1->varlocal, " in  brentgd  ");
	...
	fu=f1dim(c1, u);
		...
		*xmin = bx;		
		...
		if ((fabs(fc - fa) / fc)  < 0.000001) {
			 write_check(c1->varlocal, " ending  brentgd  ");
			return(fb);
		}
  ...
  //  nrerror("Too many iterations in brent");
	 write_check(c1->varlocal, " quiting brentgd   too many times around ... ");
	return(fb);
}
