...
///////////////////////////////////
{
	 ...
	 write_check(c1->varlocal, " in fvalue  ");
        ...
        for (i = 1;i<= c1->numberOfParameterAffected; i++)
        {
				...
				/*  sprintf(buf1, "%f", lot1[i]);
				if ( System.Double.isNaN(lot1[i]) == true) {
					write_check(c1->varlocal, "Error in calc11 fvalue");
					return(lot1[i]);
				}
				*/
        ...
        if ((ok == FALSE) || (c1->CALC_CANCEL == true)) 
        {
...
/*		if(  System.Double.isNaN(partial) == true) { 
				write_check(c1->varlocal, "Partial is not a number ");
		}
		*/ 
   ...
   write_check(c1->varlocal, "exiting dif1_ci");   //  gwwd 10-1 XXX
...
#define  MESSAGE_SIZE 400
	...
	sprintf(buf1, "value in calc11 fvalue %f", fp);
			 {
					write_check(c1->varlocal, buf1);
	...
	dif1_ci_2(c1, c1->valueOfParameterAffected, xi);       //  here is the call to differential ...
			for(i=1; i<=c1->numberOfParameterAffected; i++) 
			 {		sprintf(buf1, "dif value in calc11 fvalue %i  %f", i, xi[i]);
					write_check(c1->varlocal, buf1);
