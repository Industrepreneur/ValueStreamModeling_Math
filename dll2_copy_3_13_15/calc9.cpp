                                        ...
                                        float*          tbatchValue)            // original valye of tbatch for each part
{
	...
	if (outfile.is_open()== false) {
	 ...
	 outfile.close();  //  gwwd error 9-26
        ...
        {
                sprintf(buf1, " %s %s ", "No products to optimize", "In MPX optimization");
            ...
            for (i=0; i<numberOfParts; i++)
            {
                    ...
                    {//   flag = 2  lot changes and tbatch = -1
                     //        => tbatch is changing with lotsize
                            ...
                            tpart->tbatch = -1;
							sprintf	(buf1, "ith %i part   pmtr %i  part %s  lotsize hold tbatch=lot ", i, j,  tpart->name);
							write_check(c1->varlocal, buf1);
                    ...
                    else if(optimizeLotSize[i]!=0)
                    {
							...
							sprintf	(buf1, "ith %i part   pmtr %i  part %s  lotsize tbatch=fixed ", i, j,  tpart->name);
							write_check(c1->varlocal, buf1);
                    ...
                    if(optimizeTbatch[i]!=0)
                    {
							...
							sprintf	(buf1, "para %i  part %i  tbatch val ", i, tpart->name);
							write_check(c1->varlocal, buf1);
                            j++;
                    }
