...
/*************************************************/
{
		...
		sprintf(buf1, " f checking where lpiece  5 %s %f \n", toper->name, toper->lpiece);
		write_check(c1->varlocal, buf1);
		toper = c1->get_oper_num(8);
		sprintf(buf1, "checking where lpiece 8 %s %f \n", toper->name, toper->lpiece);
		write_check(c1->varlocal, buf1);
		...
		i_oper = o_start +3;
         ...
         for (i_oper= o_start+3; i_oper< o_start+o_count; i_oper++ )                
                ...
                xer = (toper->erlot + toper->erbatch*batches + toper->epiece*toper->lsize) * eq->facrun;
                        ...
                        if ((xlr > xer) ||
                            (xls > xes)) {
                                sprintf_s (buf1, "WARNING: #(83)\n Equipment Time  setup %f run %f is less than Labor Time setup %f run %f for an operation. Be sure you want more than 1 person assigned to the operation. %f %f   %f %f   %f %f   %f ", xes, xer, xls, xlr, toper->lrlot, toper->lrbatch, batches, toper->lpiece, toper->lsize, lab->facrun, eq->facrun);
                                warn_err ( c1, buf1, 1, lab->name, eq->name, tpart->name, toper->name, " ", " ");

								toper = c1->get_oper_num(4);
								sprintf(buf1, "e checking where lpiece  5 %s %f \n", toper->name, toper->lpiece);
								write_check(c1->varlocal, buf1);
								toper = c1->get_oper_num(8);
								sprintf(buf1, "checking where lpiece 8 %s %f \n", toper->name, toper->lpiece);
								write_check(c1->varlocal, buf1);
