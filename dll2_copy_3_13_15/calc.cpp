...
/* ---------------------------------------------- */
{
	...
	int i_count;
        ...
        if (c1->first_part_ptr() == NULL) {
                Message(c1, "Cannot Calculate anything.\n No Parts have been added yet.");
				//logfile << "no parts\n";
				//logfile.flush();
                return;
        };
		...
		//write_check(c1->varlocal, "at start do_all _visits");
        do_all_visits (c1);
		//logfile << "after do_all_visits\n";
		//logfile.flush();
		write_check(c1->varlocal, "after do_all _visits");
      ...
      //  if (c1->CALC_CANCEL == true) return;
		...
		//logfile << "after mpc\n";
		//logfile.flush();
		write_check(c1->varlocal, " mpc end");
